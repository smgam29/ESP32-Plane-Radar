...
def build(version, output):
    ...
    story = [p("Plane Radar user manual", TITLE),
      ...
      p("More than one radar", HEAD),
      p("All units deliberately use the friendly name <b>plane-radar.local</b>. That name cannot reliably select among several units. Use each radar's individual IP address and bookmark it."),
      p("Status panel", HEAD),
